...
export default function DWTUserInterface(props) {
  ...
  useEffect(() => {
    ...
    if (prevProps.status !== props.status) {
      ...
      } else handleOutPutMessage(_text);
    }
    if (
      prevProps.buffer.current !== props.buffer.current ||
      props.buffer.updated
    ) {
  ...
  const handleBarcodeResults = (results) => {
    if (results === "clear") setBarcodeRects([]);
    else {
      let _oldBR = [...barcodeRects];
      if (results.length > 0) {
        let zoom;
        if (
          props.runtimeInfo.showAbleWidth >= props.runtimeInfo.ImageWidth &&
          props.runtimeInfo.showAbleHeight >= props.runtimeInfo.ImageHeight
        ) {
          zoom = 1;
        } else if (
          props.runtimeInfo.showAbleWidth / props.runtimeInfo.showAbleHeight >=
          props.runtimeInfo.ImageWidth / props.runtimeInfo.ImageHeight
        ) {
          zoom =
            props.runtimeInfo.showAbleHeight / props.runtimeInfo.ImageHeight;
        } else {
        ...
        for (let i = 0; i < results.length; ++i) {
          ...
          let leftBase =
            1 +
            props.runtimeInfo.showAbleWidth / 2 -
            (props.runtimeInfo.ImageWidth / 2) * zoom;
          let topBase =
            1 +
            props.runtimeInfo.showAbleHeight / 2 -
            (props.runtimeInfo.ImageHeight / 2) * zoom;
    ...
    <div id="DWTcontainer" className="container">
      ...
      >
        ...
        <div className="DWT_Notice">
          <p>
            <strong>Platform &amp;Browser Support:</strong>
          </p>
          Chrome|Firefox|Edge on Windows
          <p>
            <strong>OCR:</strong>{" "}
          </p>{" "}
          Only English with OCR Basic is demonstrated.
          <br />
          Click &nbsp;
          <u>
            <a href="https://www.dynamsoft.com/Products/ocr-basic-languages.aspx">
              here
            </a>
          </u>
          &nbsp;for other supported languages and&nbsp;
          <u>
            <a href="https://www.dynamsoft.com/Products/cpp-ocr-library.aspx">
              here
            </a>
          </u>{" "}
          for the differences betwen two available OCR engines.
        </div>
      </div>
    </div>
  );
